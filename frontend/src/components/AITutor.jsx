...
import { aiService, tutorService, usageService } from "../services/api";
...
const QUICK = [
  ...
  {
    label: "Fix my code",
    prefill: "Fix this — here's the code and the error:\n\n",
  },
  {
    label: "Explain more",
    send: "Can you explain that in more detail, with a code sample?",
  },
];
...
const SLASH = [
  {
    name: "session",
    desc: "Browse & switch to a past chat",
    aliases: ["sessions", "resume", "history"],
  },
  { name: "new", desc: "Start a fresh chat", aliases: ["clear", "reset"] },
  {
    name: "usage",
    desc: "Show AI token usage — session & weekly",
    arg: "[free|pro]",
  },
...
function AITutor({ language, context, embedded = false, persist = false }) {
  ...
  const compact = async (auto = false) => {
    ...
    const items = messages.filter(
      (m) => m !== GREETING && !m.streaming && !m.sys,
    );
    ...
    try {
      ...
      const r = await tutorService.compact(
        turns,
        sessionRef.current,
        KEEP_RECENT,
      );
  ...
  const runSlash = async (raw) => {
    ...
    if (name === "plan" || (name === "usage" && arg)) {
      ...
      try {
        ...
        pushSys(
          `Switched to **${r.data.plan_label}**.\n\n${usageMarkdown(r.data)}`,
        );
      } catch (e) {
        pushSys(
          `Couldn't switch plan${e?.response?.data?.detail ? ` — ${e.response.data.detail}` : ""}.`,
        );
      }
      return;
    }
    ...
    try {
      ...
      for (;;) {
        ...
        for (const frame of frames) {
          ...
          else if (evt.kind === "reply" || evt.kind === "done")
            acc = evt.response || acc;
        }
      }
      streamDone = true;
    } catch {
      ...
      if (!acc) {
        try {
          ...
          acc =
            "I couldn't reach the tutor. Try again in a moment — and paste the exact error text when you do.";
        }
      }
      streamDone = true;
      finish();
  ...
  const onKeyDown = (e) => {
    if (slashMenu.length > 0) {
      ...
      if (e.key === "Tab") {
        e.preventDefault();
        setInput(`/${slashMenu[menuIdx].name} `);
        return;
      }
      if (e.key === "Escape") {
        e.preventDefault();
  ...
  return (
    <div
      className={`relative flex flex-col ${embedded ? "h-full" : "h-[440px]"}`}
    >
      ...
      {showSessions && (
        <div className="absolute inset-0 z-30 flex flex-col bg-cs-darkest/95 backdrop-blur-xl">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-cs-line/10">
            ...
            <span className="font-mono text-sm font-semibold flex-grow">
              Past chats
            </span>
            <button
              onClick={newChat}
            ...
            <button
              ...
              className="p-1.5 text-cs-text-muted hover:text-cs-text"
            >
              <FiX />
            </button>
          </div>
            ...
            {(sessions || []).map((s) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-sm font-semibold truncate flex-grow">
                    {s.title}
                  </span>
                  <span className="font-mono text-[10px] text-cs-text-muted shrink-0">
                ...
                <p className="text-[11px] text-cs-text-dim truncate mt-1 pl-5">
                  {s.preview}
                </p>
                <p className="text-[10px] text-cs-text-muted font-mono mt-0.5 pl-5">
                  {s.turns} messages
                </p>
              </button>
            ))}
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-cs-primary/20 bg-cs-overlay/[0.05]">
          ...
          <div className="flex-grow">
            <div className="font-mono text-sm font-semibold text-cs-text">
              <span className="text-cs-mint select-none">❯&nbsp;</span>
              codesquare_agent --live
            </div>
      ...
      <div
        ref={scrollRef}
        className={`flex-grow overflow-y-auto p-4 space-y-4 ${embedded ? "min-h-0" : ""}`}
      >
          ...
          msg.role === "user" ? (
            <div key={index} className="flex justify-end">
              <div className="max-w-[88%] w-fit rounded-lg px-4 py-2.5 text-[13px] leading-6 font-mono whitespace-pre-wrap bg-cs-primary/15 text-cs-primary border border-cs-primary/30 shadow-[0_0_18px_-10px_rgb(var(--cs-primary)/0.7)]">
                <span className="text-cs-mint font-bold select-none">
                  ❯&nbsp;
                </span>
                {msg.content}
              </div>
            ...
            <div key={index} className="flex justify-start">
              <div className="max-w-full w-full text-sm rounded-lg border border-cs-line/15 bg-cs-overlay/[0.04] px-4 py-3">
                ...
                <Markdown
                  text={msg.content}
                  size="text-[13px]"
                  className="space-y-1 text-cs-text-dim"
                />
              </div>
            </div>
          ) : (
              ...
              <div className="max-w-full w-full text-sm">
                ...
                <>
                  <Markdown
                    text={msg.content}
                    size="text-[13px]"
                    className="space-y-1 leading-6 text-cs-text-dim"
                  />
                  {msg.streaming && (
      ...
      <div className="border-t border-cs-line/10 p-3 space-y-2 relative">
            ...
            {slashMenu.map((c, i) => (
              <button
                ...
                className={`w-full flex items-baseline gap-2 px-3 py-2 text-left transition-colors ${
                  i === menuIdx
                    ? "bg-cs-primary/12"
                    : "hover:bg-cs-overlay/[0.05]"
                }`}
              >
                <span className="font-mono text-sm text-cs-primary shrink-0">
                  /{c.name}
                  {c.arg && (
                    <span className="text-cs-text-muted"> {c.arg}</span>
                  )}
                </span>
        ...
        <div className="flex flex-wrap gap-1.5">
          {QUICK.map((q) => (
            ...
            >
              <span className="text-cs-green select-none">$ </span>
              {q.label}
            </button>
          ))}
        </div>
        ...
        >
          ...
          <div className="flex items-start gap-2 px-3.5 pt-3 pb-1.5">
            <span className="text-cs-green font-mono text-[13px] select-none shrink-0 leading-6">
              $
            </span>
            <textarea
              ref={taRef}
          ...
          <div className="flex items-center gap-1.5 px-2 py-2 border-t border-cs-line/10">
            ...
            >
              <FiSend className="text-cs-primary" />{" "}
              <span className="hidden sm:inline">run</span>
            </button>
          </div>
        </form>
        <p className="text-[10px] text-cs-text-muted/70 mt-1 font-mono">
          Try <span className="text-cs-primary">/session</span>{" "}
          <span className="text-cs-primary">/new</span>{" "}
          <span className="text-cs-primary">/usage</span>
        </p>
      </div>
    </div>
  );
